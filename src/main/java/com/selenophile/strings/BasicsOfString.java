...
public class BasicsOfString {
    ...
    public String reverseStringUsingConcat(String originalString) {
        String updatedString = "";
        int lengthOfOriginalString = originalString.length();
        for (int i = lengthOfOriginalString - 1; i >= 0; i--) {
            char ch = originalString.charAt(i);
            updatedString = updatedString.concat(Character.toString(ch));
        }
    ...
    * Example DOD is a Palindrome String
    * */
    public boolean checkPalindrome(String value){
        return reverseStringUsingConcat(value.toLowerCase()).equals(value.toLowerCase());
    }
}
